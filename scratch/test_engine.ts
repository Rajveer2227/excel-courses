...
async function runFullVerificationSuite() {
    ...
    try {
        ...
        const campaignData = {
            ...
            parsedContacts: [
                { phone: '9823045678', name: 'John Doe', status: 'Pending' },
                { phone: '9422411223', name: 'Jane Smith', status: 'Pending' },
                { phone: '9890088776', name: 'Alex Johnson', status: 'Pending' }
            ]
        };

        const createRes = await createCampaignInDb(campaignData as any);
        if (!createRes) {
            throw new Error(`Campaign creation returned null`);
        }
        console.log("   ✓ CREATE Campaign API PASSED:", createRes.campaignName);

        // Verify recipient rows in database
        const recipientsInDb = await sql`
            SELECT id, COALESCE(phone_number, phone_e164) as phone_number, COALESCE(contact_name, recipient_name) as contact_name, status 
        ...
        if (!fetchRes1 || fetchRes1.campaignName !== campaignData.campaignName) {
        ...
        if (!updateRes) {
            throw new Error(`Auto-save update returned null`);
        }
        ...
        if (!fetchRes2 || fetchRes2.campaignName !== updatedName || fetchRes2.notes !== updatedNotes) {
        ...
        if (!dupRes) {
            throw new Error(`Campaign duplication returned null`);
        }
        const duplicatedId = dupRes.id;
        console.log("   ✓ Duplicated Campaign Created in DB:", dupRes.campaignName);

        const dupFetch = await getCampaignByIdFromDb(duplicatedId);
        if (!dupFetch || dupFetch.campaignName !== `${updatedName} Copy`) {
        ...
        if (!schedRes) {
            throw new Error(`Scheduling returned null`);
        }

        const schedFetch = await getCampaignByIdFromDb(testId);
        if (!schedFetch || schedFetch.status !== 'Scheduled' || !schedFetch.scheduledAt) {
            throw new Error(`Expected status Scheduled, got ${schedFetch?.status}`);
        }
        auditVerified.push("Scheduled");
        await logAuditInDb("Scheduled", { campaignId: testId, scheduledAt: schedFetch.scheduledAt });
        ...
        if (!archiveRes) {
            throw new Error(`Archiving returned null`);
        }
        ...
        const foundInActive = activeList1.data?.find((c: any) => c.id === testId);
        if (foundInActive) {
        ...
        const foundInArchived = archivedList.data?.find((c: any) => c.id === testId);
        if (!foundInArchived) {
        ...
        if (!deleteRes) {
            throw new Error(`Soft delete returned null`);
        }
        ...
        if (!searchResName.data?.some((c: any) => c.campaignName.includes("Python Full Stack"))) {
        ...
        if (!searchResTag.data?.some((c: any) => c.tags.includes("Python"))) {
        ...
        if (!searchResStatus.data?.some((c: any) => c.status === "Completed")) {
        ...
        const nameList = sortResName.data.map((c: any) => c.campaignName);
        const isSorted = nameList.every((val: string, i: number, arr: string[]) => !i || arr[i - 1].localeCompare(val) <= 0);
        if (!isSorted) {
        ...
        if (searchCmp1) await deleteCampaignInDb(searchCmp1.id);
        if (searchCmp2) await deleteCampaignInDb(searchCmp2.id);

        // ----------------------------------------------------
        // STEP 9: 25 CAMPAIGNS PAGINATION STRESS TEST
        // ----------------------------------------------------
        console.log("\n[STEP 9] Seed 25 Campaigns & Test Pagination (3 Pages)...");
        const uniqueTag = `StressTest_${Date.now()}`;
        ...
        const p1 = await getCampaignsFromDb({ tag: uniqueTag, page: 1, limit: 10 });
        const p2 = await getCampaignsFromDb({ tag: uniqueTag, page: 2, limit: 10 });
        const p3 = await getCampaignsFromDb({ tag: uniqueTag, page: 3, limit: 10 });
        ...
        const combinedIds = [...p1.data.map((c: any) => c.id), ...p2.data.map((c: any) => c.id), ...p3.data.map((c: any) => c.id)];
        ...
        console.log("     - Total Campaigns:", statsRes.totalCampaigns);
        console.log("     - Drafts:", statsRes.draft);
        console.log("     - Scheduled:", statsRes.scheduled);
        console.log("     - Completed:", statsRes.completed);
        console.log("     - Archived:", statsRes.archived);
        console.log("     - Total Recipients:", statsRes.totalRecipients);

        console.log("\n=================================================");
        console.log("    ALL VERIFICATION CHECKLIST ITEMS PASSED     ");
        console.log("=================================================");
